<template>
  ...
  <Search></Search>
  <div class="container">
    <div class="block-title" :style="isDark(store.switchDark)">NFT</div>
    ...
    <div class="table" :style="store.switchDark ? 'background:#202020' : 'background:#f2f2f2'">
      ...
      <el-table :class="store.switchDark ? 'black' : 'white'" :data="tableData" v-loading="loading">
        <el-table-column prop="type_tag" :label="$t('lang.CoinID')" width="350" :show-overflow-tooltip="true">
          <template #default="scope">
            <router-link :to="'/nftTokenInfo?tag=' + scope.row.type_tag" :style="isDark(store.switchDark)">
              {{ scope.row.type_tag }}
            </router-link>
          </template>
        </el-table-column>
        <el-table-column prop="collection" :label="$t('lang.Collection')" />
        <el-table-column prop="create_at" :label="$t('lang.CreateAt')" width="200">
          <template #default="scope">
            <span v-if="scope.row.create_at != null">{{ timestampToTime(scope.row.create_at) }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="creator" :label="$t('lang.Creator')" width="210">
          <template #default="scope">
        ...
        <el-table-column prop="supply" :label="$t('lang.Supply')" />
      </el-table>
...
import { nftTokenList } from '@/http/api/index.ts'
...
export default defineComponent({
  ...
  setup() {
    const data = reactive({
      ...
      tableData: [],
      getNftTokenList: () => {
        nftTokenList({ page: data.currentPage, count: data.pageSize })
          .then((res: any) => {
            console.log('nftToken', res)
